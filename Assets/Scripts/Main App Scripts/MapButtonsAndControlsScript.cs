...
public class MapButtonsAndControlsScript : MonoBehaviour, IPointerDownHandler, IDragHandler
{
    ...
    private void ClampPosition()
    {
        ...
        Vector2 scaledMapSize = new Vector2(mapRect.width * rectTransform.localScale.x, mapRect.height * rectTransform.localScale.y);

        Vector2 pos = rectTransform.anchoredPosition;

        // Because pivot is center (0.5, 0.5), calculate edges accordingly
        ...
        // Limits so map edges do not go outside container edges
        float minX = halfParentWidth - halfMapWidth;  // Negative if map bigger than container
        float maxX = halfMapWidth - halfParentWidth;  // Positive if map bigger

        float minY = halfParentHeight - halfMapHeight;
        float maxY = halfMapHeight - halfParentHeight;

        // If map smaller than container, center it by fixing min and max equal
