...
public class CompassNavigationArrow : MonoBehaviour
{
    ...
    private void UpdateArrowRotation()
    {
        ...
        if (currentLocalizationMode == LocalizationMode.GPS)
        {
            // GPS Mode: Calculate bearing using compass
            Vector2 targetLocation = new Vector2(targetNode.latitude, targetNode.longitude);
            float bearing = CalculateBearingGPS(userLocation, targetLocation);
            
            // Adjust for compass heading
            float compassHeading = Input.compass.trueHeading;
            targetAngle = bearing - compassHeading;
        }
        else
        {
            // Offline Mode: Calculate angle using X,Y coordinates
            ...
            // No compass adjustment needed in offline mode - arrow points directly to target
        }
        ...
        {
            Debug.Log($"[CompassArrow] Target: {targetNode.name}, Angle: {targetAngle:F1}°");
        }
    }
